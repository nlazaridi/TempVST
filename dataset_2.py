...
def load_list(dataset_name, data_root, mode, alternate, len_snippet):
    ...
    if mode == 'train':
        ...
        for video_num in os.listdir(video_root):
            if video_num[-3:] == 'AVI':
                continue
            idx = 0
            while idx < (len(os.listdir(os.path.join(video_root, video_num, 'images'))) - alternate * len_snippet + 1):
                video_names.append(video_num)
                index_frame.append(idx)
                idx += alternate*len_snippet + 1
    ...
    return video_names, index_frame
...
class VideoData(data.Dataset):
    def __init__(self, dataset_list, data_root, alternate, len_snippet, img_transform, mode, img_size=None, scale=None, t_transform=None, label_14_transform=None, label_28_transform=None, label_56_transform=None, label_112_transform=None ):
        ...
        if self.mode == 'train' or self.mode == 'val':
            self.video_names, self.list_num_frames = load_list(dataset_list, data_root, mode, self.alternate, self.len_snippet)
    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            ...
            idx = self.list_num_frames[index]
        ...
        for i in range(self.len_snippet):
            ...
            img = Image.open(os.path.join(path_clip, '%04d.png'%(idx+self.alternate*i+1))).convert('RGB')

            if self.mode != 'test' and i == (self.len_snippet-1):

                clip_gt = Image.open(os.path.join(path_annt, '%04d.png'%(idx+self.alternate*i+1))).convert('L')
